...
function calc() {
  ...
  function calcTotal() {
    ...
    if (sexPersone == 'female') {
      resultCalories.textContent = Math.round((447.6 + 9.2 * weightPersone + 3.1 * heightPersone - 4.3 * agePersone) * ratioPersone);
    } else {
      resultCalories.textContent = Math.round((88.36 + 13.4 * weightPersone + 4.8 * heightPersone - 5.7 * agePersone) * ratioPersone);
    }
  }
...
function cards() {
  ...
  class MenuItem {
    constructor(parentSelector, img, title, text, price) {
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getResource)('http://localhost:3000/menu').then(data => {
    data.forEach(_ref => {
      ...
      new MenuItem('.menu__field .container', img, title, descr, price, 'menu__item').createMenuItem();
    });
  }); // new MenuItem(
  // 	'.menu__field .container',
  // 	'img/tabs/vegy.jpg',
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      const object = JSON.stringify(Object.fromEntries(formData.entries())); // const object = {};
      // formData.forEach((item, key) => {
      // 	object[key] = item;
      // });
      // console.log(object);

      (0,_services_services__WEBPACK_IMPORTED_MODULE_1__.postData)('http://localhost:3000/requests', object).then(data => {
        console.log(data);
